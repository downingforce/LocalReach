...
public class SavedCharityList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCharityReference = FirebaseDatabase.getInstance().getReference(Constants.FIREBASE_CHILD_CHARITIES);
        setUpFirebaseAdapter();
    }

    private void setUpFirebaseAdapter() {
        mFirebaseAdapter = new FirebaseRecyclerAdapter<Charity, FirebaseCharityViewHolder>
                (Charity.class, R.layout.charity_list_item, FirebaseCharityViewHolder.class,
                        mCharityReference) {

            @Override
            protected void populateViewHolder(FirebaseCharityViewHolder viewHolder,
                                              Charity model, int position) {
